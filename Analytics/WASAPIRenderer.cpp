...
HRESULT WASAPIRenderer::ConfigureDeviceInternal()
{
	...
	m_DeviceProps.RingBuffer->AddChannels(m_MixFormat->nChannels, m_MixFormat->nSamplesPerSec, m_MixFormat->wBitsPerSample / 8, m_MixFormat->nSamplesPerSec, m_channels);
    ...
    else
    {
        ...
        if (SUCCEEDED( hr ))
        {
            ...
            else if (SUCCEEDED( hr ))
            {
				...
				if (m_DeviceProps.RingBuffer != nullptr && ActualBytesRead != 0)
				{
					m_DeviceProps.RingBuffer->AddData(Data, ActualBytesRead, m_channels.size(), std::vector<UINT>());
				}
