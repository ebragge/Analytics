    ...
    m_fWriting( false )
{
	...
	DeviceStateChangedEv = ref new DeviceStateChangedEvent();
    if (nullptr == DeviceStateChangedEv)
    {
...
//
HRESULT WASAPICapture::InitializeAudioDeviceAsync(int i, IMultiRingBufferProducer *buffer)
{
	m_CaptureDeviceID = i;
	...
	HRESULT hr = ActivateAudioInterfaceAsync(ID->Data(), __uuidof(IAudioClient3), nullptr, this, &asyncOp );
    if (FAILED( hr ))
    {
		DeviceStateChangedEv->SetState( DeviceState::DeviceStateInError, hr, true, m_CaptureDeviceID);
    }
...
HRESULT WASAPICapture::ActivateCompleted( IActivateAudioInterfaceAsyncOperation *operation )
{
	...
	m_inputBuffer->AddChannels(m_MixFormat->nChannels, m_MixFormat->nSamplesPerSec, m_MixFormat->wBitsPerSample/8, m_MixFormat->nSamplesPerSec, m_channels);
	...
	DeviceStateChangedEv->SetState(DeviceState::DeviceStateInitialized, S_OK, true, m_CaptureDeviceID);
...
HRESULT WASAPICapture::StartCaptureAsync()
{
    ...
    if (DeviceStateChangedEv->GetState() == DeviceState::DeviceStateInitialized)
    {
		DeviceStateChangedEv->SetState( DeviceState::DeviceStateStarting, S_OK, true,m_CaptureDeviceID );
...
HRESULT WASAPICapture::OnStartCapture( IMFAsyncResult* pResult )
{
    ...
    if (SUCCEEDED( hr ))
    {
		DeviceStateChangedEv->SetState( DeviceState::DeviceStateCapturing, S_OK, true, m_CaptureDeviceID);
...
{
    if ( (DeviceStateChangedEv->GetState() != DeviceState::DeviceStateCapturing) &&
         (DeviceStateChangedEv->GetState() != DeviceState::DeviceStateInError) )
    {
        return E_NOT_VALID_STATE;
    }

	DeviceStateChangedEv->SetState( DeviceState::DeviceStateStopping, S_OK, true, m_CaptureDeviceID);
...
HRESULT WASAPICapture::OnStopCapture( IMFAsyncResult* pResult )
{
	...
	DeviceStateChangedEv->SetState(DeviceState::DeviceStateStopped, S_OK, true, m_CaptureDeviceID);
    return S_OK;
}
...
HRESULT WASAPICapture::FinishCaptureAsync()
{
    ...
    if (DeviceStateChangedEv->GetState() == DeviceState::DeviceStateFlushing, m_CaptureDeviceID)
    {
...
HRESULT WASAPICapture::OnAudioSampleRequested( Platform::Boolean IsSilence )
{
    ...
    if ( (DeviceStateChangedEv->GetState() == DeviceState::DeviceStateStopping) ||
         (DeviceStateChangedEv->GetState() == DeviceState::DeviceStateFlushing) )
    {
        goto exit;
    }
    ...
    )
    {
        ...
        if (dwCaptureFlags & AUDCLNT_BUFFERFLAGS_DATA_DISCONTINUITY)
        {
			...
			DeviceStateChangedEv->SetState( DeviceState::DeviceStateDiscontinuity, S_OK, true, m_CaptureDeviceID);
			DeviceStateChangedEv->SetState( DeviceState::DeviceStateCapturing, S_OK, false, m_CaptureDeviceID);
			discontinuity = true;
        }
		if (dwCaptureFlags & AUDCLNT_BUFFERFLAGS_TIMESTAMP_ERROR)
		{
			String^ str = String::Concat(ID, "-TIMESTAMP_ERROR\n");
        ...
        // Store data in array
		m_inputBuffer->AddData(Data, cbBytesToCapture, m_MixFormat->nChannels, m_channels, u64DevicePosition, u64QPCPosition, discontinuity);
